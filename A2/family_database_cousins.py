"""
This program helps store rabbytes' name-age, parental relationships as well as their cousins.
"""
...
def list_cousins():
    """
    This function lists rabbytes' cousins.
    """

    while True:
        #Initialise an empty list to store cousins' names later.
        cousins_list = []
        child_name = input("Input the rabbit's name:\n")
        if child_name in list(rabbytes_name_age.keys()):
            #Use two variables to traverse the family dictionary
            for parent_name, children_list in rabbytes_family.items():
                #in this case, the name refers to a child with a definite parent as its key
                if child_name in children_list:
                    #traverse again to use the definite parent name to look for the corresponding grandparent(s)
                    for grandparent_name, parents_list in rabbytes_family.items():
                        #in this case, the name refers to a parent with a definite grandparent as its key
                        #and within a list of parental generation
                        if parent_name in parents_list:
                            #traverse the parents_list to find all uncles and aunts by excluding parents from the list
                            for peer_parent_name in parents_list:
                                #only when uncles or aunts serve as a key in the dictionary can they be cousins' parents
                                if peer_parent_name != parent_name:
                                    cousins_list += rabbytes_family.get(peer_parent_name, [])
                                
            cousins_list.sort()
            print("Cousins of", child_name + ":")
            for name in cousins_list:
                print(name)
            main_menu()
        print("That name is not in the database.")
